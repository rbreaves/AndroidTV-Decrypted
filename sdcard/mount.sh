...
while [ ! -d "/storage/emulated/0/Android" ] && [ $sdcardName != 'emulated' ] && [ $sdcardName != 'self' ]; do
	sleep 3
	sdcardName=`ls /storage/ | head -n1`
	stamp=`date "+%m-%d %T"`
...
railme() {
	...
	su -c echo "$stamp $1 - $sdcardName mounted successfully. - Manually" >> /cache/magisk.log
}
...
if [ $sdcardName != 'emulated' ] && [ $sdcardName != 'self' ]; then
	...
	su -c echo "$stamp Physical sdcard $sdcardName on retry $count failed to mount. - Manually" >> /cache/magisk.log
fi